...
class LSA(ThresBanditLearner):
  ...
  def __metrics(self) -> np.ndarray:
    """
    Returns:
      metrics of lsa for each arm
    """
    def pseudo_gap(em_mean):
      return np.abs(em_mean - self.__theta) + self.__eps

    metrics = [
        arm.total_pulls() * (pseudo_gap(arm.em_mean)**2) +
        0.5 * np.log(arm.total_pulls()) for arm in self.__pseudo_arms
    ]
    return metrics
