...
class LinUCB(LinearBanditLearner):
    ...
    def __init__(self, arm_num: int, features: List[np.ndarray],
                 delta: float, lambda_reg: float, name: str = None):
        """
        Args:
        ...
        if delta <= 0 or delta >= 1:
            raise ValueError(
                'Delta is expected greater than 0 and less than 1. Got %.2f.' % delta)
    ...
    def __LinUCB(self) -> np.ndarray:
        ...
        root_beta_t = np.sqrt(self.__lambda_reg) + np.sqrt(
            ...
            self.__d * np.log(1 + (self.__time - 1) /
