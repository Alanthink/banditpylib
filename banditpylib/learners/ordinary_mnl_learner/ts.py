...
class ThompsonSampling(OrdinaryMNLLearner):
  ...
  def __init__(self,
    ...
    if local_search_times < 10:
      logging.warning('Times of local search %d is less than 10!' %
                      local_search_times)
  ...
  def actions(self, context=None) -> List[Tuple[List[int], int]]:
    ...
    # check if still in warm start stage
      ...
      if self.__use_local_search:
        ...
        init_assortment = self.__last_actions[0][0] if \
            (not self.__first_step_after_warm_start and self.__last_actions) \
            else None
