...
def parse(config):
  actions = config['environment']['arm']['means']
  Arm = getattr(import_module(ARM_PKG),
      config['environment']['arm']['type'])
  theta = config['environment']['bandit']['param']
  arms = [Arm(action, theta) for action in actions]
  Bandit = getattr(import_module(BANDIT_PKG),
      config['environment']['bandit']['type'])
  
  bandit = Bandit(arms, theta)
